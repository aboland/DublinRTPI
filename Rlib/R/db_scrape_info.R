...
#' This is a backup for when the API is unavailable.
#' 
  ...
  for (i in 1:length(stop_numbers)) {
    ...
    if (temp_info$errorcode == 0) {
      temp_info <- 
        ...
        mutate(datatime = Sys.time() + (60 * 60), 
               stopnumber = stop_numbers[i],
               ...
               duetime = difftime(.data$arrivaldatetime, Sys.time() + (60 * 60), units = "mins") %>% 
                 round()
               )
